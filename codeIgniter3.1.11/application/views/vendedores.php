...
<?php 		foreach($puesto as $p) {?>
					<option value='<?php echo $p['ID_PUESTO']?>'><?php echo trim($p['DESCRIPCION'])?></option>
<?php 		}?>
				</select>
			</div>
...
<?php 		foreach($titulo as $t) {?>
					<option value='<?php echo $t['ID_TITULO']?>'><?php echo trim($t['DESCRIPCION'])?></option>
<?php 		}?>
				</select>
			</div>
		</div>
			...
			<tr>
				<td>
					...
					<table class="table is-hoverable" style="width:100%" id="tblVendedores">
						<col width="26%">
						<col width="29%">
						<tr ng-repeat="x in lstVendedores | orderBy:myOrder:sortDir" ng-click="selectRowVendedor($index,x.ID_CLIENTE)" ng-class="{selected: x.CLAVE === idSelCompra}">
