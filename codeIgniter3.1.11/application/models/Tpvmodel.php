...
class Tpvmodel extends CI_model
{
	...
	function get_items($desc,$tipo_req)
	{
		if($tipo_req == 'C')
		{
	...
	function get_items_vacio($tipo_req)
	{
		if($tipo_req == 'C')
		{
		...
		$query = 'SELECT P."ID_PRODUCTO","DESCRIPCION","CODIGO",to_char("PRECIO_LISTA",\'L999,999,999.00\') as "PREC_LISTA_DISP",
							"PRECIO_LISTA",
							to_char("PRECIO_COMPRA",\'L999,999,999.00\') AS "PRECIO_COMPRA_DISP",
							"PRECIO_COMPRA","IVA",
							"UNIDAD_MEDIDAD",SUM(S."STOCK") as "STOCK",
							"IMAGEN","ES_PROMO","ES_DESCUENTO","PRECIO_PROMO","PRECIO_DESCUENTO","TIPO_PS"
							FROM "PRODUCTO" as P INNER JOIN  "PRODUCTO_SUCURSAL" as S
							ON P."ID_PRODUCTO" = S."ID_PRODUCTO" '
							.$condition.
