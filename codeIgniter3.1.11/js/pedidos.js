app.controller('myCtrlPedi', function($scope,$http,$interval)
{
  ...
  $scope.getpedidos = function(){
    $http.get(pathPedi+'getpedidos/'+$scope.aniofiscal,{responseType:'json'})
        .then(res => {
  ...
  $scope.getNextDocPedido = function(){
		$http.get(pathUtils+'incremento/PEDI/'+$scope.idempresa+'/7').
		then(function(res)
		{
			if(res.data.length > 0)
			{
  ...
  $scope.buscaprodbycodigo = function(event)
  {
    ...
    $scope.selectProdBus = function(idxRowListaBusq)
    {
      ...
      $http.get(pathTpv+'getitemsbyprodsuc/'+$scope.producto.id_producto+'/'+$scope.idsucursal).
      then(function(res)
      {
        if(res.data.length > 0)
        {
    ...
    { 
      ...
      var dataPedido =
      {
        ...
        fechapedido:formatDateInsert(new Date()),
        aniofiscal:$scope.aniofiscal,
        idempresa:$scope.idempresa,
        idformapago:'01', //$scope.fact.formapago,
        importe:$scope.total,
        idsucursal:$scope.idsucursal
      }

      $http.put(pathPedi+'registrapedido',dataPedido).
      then(function(res)
      {
