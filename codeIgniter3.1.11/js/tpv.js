app.controller("myCtrlTpv", function ($scope, $http, $interval, $routeParams) {
  ...
  $scope.init = function () {
    ...
    var hoy = new Date();
    $scope.fechaCorte = formatReporte($scope.fechaCorteTmp);
    $http
      .get(pathAcc + "getdata", { responseType: "json" })
      .then(function (res) {
  ...
  $scope.getIdClienteVentasMostrador = () => {
    $http
      .get(pathClte + "getidvtsmostr/" + $scope.idempresa)
      .then((res) => {
        if (res.data) {
  ...
  $scope.getUsuario = () =>{
    $http.get(pathUsr+'getusrbyid/'+$scope.idusuario)
    .then(res =>{
      if(res.data){
        $scope.isAdmin = res.data[0].NOMBRE === ADMIN;
      }
  ...
  $scope.getNextDocTpv = function () {
    $http
      .get(pathUtils + "incremento/TPVS/" + $scope.idempresa + "/7")
      .then(function (res) {
  ...
  $scope.getNextDocFact = function () {
    $http
      .get(pathUtils + "incremento/FACT/" + $scope.idempresa + "/0")
      .then(function (res) {
        if (res.data.length > 0) {
  ...
  $scope.getMoneda = function () {
    $http
      .get(pathUtils + "getmoneda", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.getMetPago = function () {
    $http
      .get(pathUtils + "getmetpag", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.getFormPago = function () {
    $http
      .get(pathUtils + "getformpag", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.getTipoPago = function () {
    $http
      .get(pathUtils + "gettipopago", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
          $scope.lstTipopago = res.data;
          $scope.fact.tipopago =
            $scope.cliente.dcredito == 0 || $scope.cliente.dcredito == ""
              ? 1
              : 2;
        }
      })
  ...
  $scope.getUsoCfdi = function () {
    $http
      .get(pathUtils + "getusocfdi", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.getvendedores = () => {
    $http
      .get(pathVend + "getvendedores/" + $scope.fact.idempresa + "/vacio", {
        responseType: "json",
      })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.permisos = function () {
    $http
      .get(pathUsr + "permusrproc/" + $scope.idUsuario + "/" + $scope.idProceso)
      .then((res) => {
  ...
  $scope.getbanco = function () {
    $http
      .get(pathUtils + "getbancos", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.gettarjetas = function () {
    $http
      .get(pathUtils + "gettarjetas", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.getvales = function () {
    $http
      .get(pathUtils + "getvales", { responseType: "json" })
      .then((res) => {
        if (res.data.length > 0) {
  ...
  $scope.setSelectedDscnt = function (indexRowCompra) {
    $scope.indexRowCompra = indexRowCompra;
    $scope.proddscnt.producto =
      $scope.lstProdCompra[indexRowCompra].DESCRIPCION;
      ...
      $http
        ...
        .then(function (res) {
          if (res.data != false) {
            ...
            swal(
              "No existe un producto y/o servicio con el código " +
                $scope.codigo_prodto
            );
          }
        })
  ...
  $scope.buscprodbydesc = function (event) {
    ...
    $http
      .get(pathTpv + "getitems/" + $scope.idempresa + "/" + searchword + "/V", {
        responseType: "json",
      })
      .then(function (res) {
        if (res.status == "200") {
  ...
  $scope.agregaProducto = function () {
    ...
    if (
      Number($scope.cantidad) > Number($scope.cantProd) &&
      $scope.tipo_ps == "P"
    ) {
      swal(
        "La cantidad de productos [" +
          $scope.cantidad +
          "] seleccionados es mayor a la existencia",
        "En existencia[" + $scope.cantProd + "]",
        "warning"
      );
    ...
    importe = $scope.esDscto
      ? Number($scope.cantidad * $scope.precio * (1 - $scope.descuento / 100))
      : Number($scope.cantidad * $scope.precio);
    var dataCompra = {
  ...
  $scope.calculaValoresMostrar = () => {
    ...
    for (var i = 0; i < $scope.lstProdCompra.length; i++) {
      ...
      $scope.dsctoValor +=  Number($scope.lstProdCompra[i].CANTIDAD) * Number($scope.lstProdCompra[i].PRECIO_LISTA) * ($scope.lstProdCompra[i].ESDSCTO
          ? Number($scope.lstProdCompra[i].DESCUENTO / 100) : 0);
  ...
  $scope.seleccionaCliente = function (indxRowClte) {
    ...
    $scope.rfc_cliente =
      $scope.lstCliente[indxRowClte].RFC != null
        ? $scope.lstCliente[indxRowClte].RFC.trim()
        : "";
    //$scope.fact.usocfdi = $scope.lstCliente[indxRowClte].USO_CFDI;
    $scope.cliente.id_uso_cfdi = Number(
      $scope.lstCliente[indxRowClte].ID_USO_CFDI
    );
    ...
    $scope.cliente.dcredito = Number(
      $scope.lstCliente[indxRowClte].DIAS_CREDITO
    );
    $scope.cliente.id_forma_pago = Number(
      $scope.lstCliente[indxRowClte].ID_FORMA_PAGO
    );
  ...
  $scope.buscacodcliente = (event) => {
    if (event.keyCode == 13) {
      $http
        .get(
          pathClte + "findbycode/" + $scope.claveclte + "/" + $scope.idempresa
        )
        .then((res) => {
          if (res.data) {
            $scope.claveclte = res.data[0].CLAVE.trim();
            $scope.nombre_cliente = res.data[0].NOMBRE;
            $scope.rfc_cliente =
              res.data[0].RFC != null ? res.data[0].RFC.trim() : "";
            $scope.cliente.id_uso_cfdi = Number(res.data[0].ID_USO_CFDI);
            $scope.fact.usocfdi = Number(
              $scope.lstCliente[indxRowClte].ID_USO_CFDI
            );
            ...
            swal(
              "No existe el cliente con codigo " +
                $scope.claveclte +
                ", puede hacer la búsqueda por nombre"
            );
            $scope.nombre_cliente = "";
            $scope.claveclte = "";
            return;
  ...
  $scope.buscavendedor = function (event) {
    var searchword;
    searchword =
      $scope.nombre_vendedor != "" ? $scope.nombre_vendedor : "vacio";
    $http
      .get(pathVend + "getvendedores/" + $scope.idempresa + "/" + searchword)
      .then(function (res) {
        if (res.data.length > 0) {
      ...
      $http
        ...
        .then((res) => {
          if (res.data) {
            ...
            swal(
              "No existe el vendedor con codigo " +
                $scope.idvendedor +
                ", puede hacer la búsqueda por nombre"
            );
            $scope.idvendedor = "";
  ...
  $scope.registraCompra = function () {
    if (
      ...
      swal("La cantidad a cobrar es mayor que el Importe Total",
        "Verifique las cantidades","warning"
      );
      return;
    }
    if (
    ...
    if ($scope.fact.tipopago == 2 || $scope.fact.req_factura) {
      $http
        .post(pathFactura + "savefactura", dataFact)
        .then((res) => {
          dataVenta.idfactura = res.data[0].registra_factura;
          $scope.fact.idfactura = dataVenta.idfactura;
          //este indica que ya se timbro
          dataVenta.facturado = "false";
          $http
            ...
            .then(function (res) {
              ...
              $scope.limpiaCompra();
              swal(
                "La venta se registro exitosamente",
                "Felicidades!",
                "success"
              );
            })
  ...
  $scope.registraFactura = () => {
    ...
    $scope.fact.metodopago =
      $scope.lstMetpago[$scope.fact.metodopago - 1].MET_PAGO;
    $scope.fact.usocfdicodigo =
      $scope.lstUsocfdi[$scope.fact.usocfdi - 1].CLAVE;
    $scope.fact.formapago = $scope.cliente.formapago;
    $scope.fact.folio = $scope.fact.documento;
    $http
      .post(pathCreaFact + "creacfdi", $scope.fact)
      .then((res) => {
  ...
  $scope.VerificarCliente = () => {
    if ($scope.claveclte != "") {
      $http
        .get(pathClte + "loadbyidverfi/" + $scope.idcliente, {
          responseType: "json",
        })
        .then(function (res) {
          if (res.data.length > 0) {
  ...
  $scope.enviaDatosCliente = () => {
    ...
    if ($scope.btnVerifClte == "Agregar") {
      $http
        .get(pathUtils + "incremento/CLTE/" + $scope.idempresa + "/4")
        .then(function (res) {
          $scope.cliente.clave = res.data[0].VALOR;
          $http
            .post(pathClte + "save", $scope.cliente)
            .then(function (res) {
      ...
      $http
        .put(pathClte + "updatecliente/" + $scope.idcliente, $scope.cliente)
        .then(function (res) {
  ...
  $scope.verificaExistencia = () => {
    ...
    $http
      .get(pathTpv + "getproductosforsucursal/" + $scope.id_producto, {
        responseType: "json",
      })
      .then(function (res) {
        if (res.data.length > 0) {
  ...
  $scope.calculaDescInd = () => {
    ...
    if ($scope.proddscnt.descuento > 10) {
      swal(
        "No puede dar un descuento mayor al 10%",
        "Pida ayuda a su superior",
        "error"
      );
    }
    $scope.lstProdCompra[$scope.indexRowCompra].IMPORTE =
      $scope.lstProdCompra[$scope.indexRowCompra].PRECIO_LISTA *
      $scope.lstProdCompra[$scope.indexRowCompra].CANTIDAD *
      (1 - $scope.proddscnt.descuento / 100);
    $scope.lstProdCompra[$scope.indexRowCompra].DESCUENTO =
      $scope.proddscnt.descuento;
    $scope.lstProdCompra[$scope.indexRowCompra].ESDSCTO =
      $scope.proddscnt.descuento > 0;
    $scope.calculaValoresMostrar();
  };
  ...
  $scope.calculaDescTodos = () => {
    $scope.lstProdCompra.map((prod) => {
      ...
      prod.IMPORTE =
        prod.PRECIO_LISTA *
        prod.CANTIDAD *
        (1 - $scope.proddscnt.descuentoTodos / 100);
    });
    $scope.calculaValoresMostrar();
  };
  ...
  $scope.abreOperaciones = () => {
    ...
    var hoy = $scope.fechaCorteTmp; //new Date();
    ...
    $http
      .get(pathTpv +
          "getdataoper/" +$scope.idempresa +"/" +$scope.aniofiscal +"/" +(mes + "-" + dia + "-" + year + " 00:00:00") +
          "/" +(mes + "-" + dia + "-" + year + " 23:59:59")
      )
      .then((res) => {
  ...
  $scope.corteCaja = () => {
    ...
    swal({
      title:
        "Se va a realizar el corte de caja, se va generar la factura " +
        $scope.fact.documento,
      text: "Continuar?",
      icon: "warning",
      buttons: [true, true],
      dangerMode: true,
      ...
      if (yes) {
        for (var i = 0; i < $scope.lstVentas.length; i++) {
          if (
            $scope.lstVentas[i].CORTECAJA === "f" &&
            $scope.lstVentas[i].CANCELADO === "f" 
          ) 
          {
            corteCaja.importe += $scope.lstVentas[i].IMPORTE;
          }
        }
        if (corteCaja.importe > 0) {
          corteCaja.operaciones = $scope.lstVentas.length;
          $http
            .post(pathTpv + "savecortecaja", corteCaja)
            .then((res) => {
              ...
              for (var i = 0; i < $scope.lstVentas.length; i++) {
                ...
                $http.put(
                    pathTpv +"corteventa/" +corteCreado + "/" +$scope.lstVentas[i].ID_VENTA
                  )
                  .then((res) => {
              ...
              var dataFact = {
                documento: $scope.fact.documento,
                ffactura: formatDateInsert($scope.fechaCorteTmp), //formatDateInsert(new Date()),
                idcliente: 0,
                importe: 0,
                saldo: 0, //$scope.total,
                tipopago: 1, //$scope.fact.tipopago,
                frevision: null, //formatDateInsert(new Date()),
                fvencimiento: null, //$scope.cliente.dcredito>0?formatDateInsert(nextdate):null,
                idvendedor: 1, //$scope.idvendedor =='' ? null : $scope.idvendedor,
                idempresa: $scope.idempresa,
                aniofiscal: $scope.aniofiscal,
                idsucursal: $scope.idsucursal,
                formapago: 1, //$scope.fact.req_factura ? $scope.cliente.formapago : null,
                usocfdi: 1, //$scope.fact.req_factura ? $scope.fact.usocfdi : null,
                metodopago: 1, //$scope.fact.req_factura ? $scope.fact.metodopago : null,
                contacto: null,
                idmoneda: 1,
                idpedido: null,
              ...
              for (var i = 0; i < $scope.lstVentas.length; i++) {
                if (
                  $scope.lstVentas[i].CORTECAJA === "f" &&
                  $scope.lstVentas[i].CANCELADO === "f" &&  $scope.lstVentas[i].ID_FACTURA === null
                ) 
                {
                  importeFactura += $scope.lstVentas[i].IMPORTE;
                }
              }
              dataFact.importe = importeFactura;
              $http.post(pathFactura + "savefactura", dataFact)
                .then((fact) => {
                  var x = 1;
                  for (var j = 0; j < $scope.lstVentas.length; j++) {
                    if (
                      $scope.lstVentas[j].CORTECAJA === "f" &&
                      $scope.lstVentas[j].CANCELADO === "f" &&
                      $scope.lstVentas[j].ID_FACTURA === null
                    ) 
                    {
                      var dataFacturaProd = {
                        idfactura: fact.data[0].registra_factura,
                        clave: "",
                        nombre: "",
                        importe: "",
                        iva:0
                      };
                      dataFacturaProd.clave = $scope.lstVentas[j].DOCUMENTO;
                      dataFacturaProd.nombre = "VENTA " + (x++);
                      dataFacturaProd.importe = $scope.lstVentas[j].IMPORTE;
                      dataFacturaProd.iva = $scope.lstVentas[j].IVA !== null ? $scope.lstVentas[j].IVA : $scope.lstVentas[j].IMPORTE * .16;
                      $http
                        .post(pathTpv + "saveproductofact", dataFacturaProd)
                        .then((resfac) => {
                          
                        })
                        .catch((errorFact) => {
                          console.log(errorFact);
                        });
                    }
                  }
  ...
  $scope.eliminaOperacion = () => {
    swal({
      title:
        "Esta seguro que desea eliminar la operación, una vez hecho esto, no se podrá recuperar!",
      text: "Continuar?",
      icon: "warning",
      buttons: [true, true],
      dangerMode: true,
      ...
      if (answer) {
        $http
          .delete(pathTpv + "deloperacion/" + $scope.idOpSel)
          .then((res) => {
            $scope.abreOperaciones();
