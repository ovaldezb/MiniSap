app.controller('myCtrlCompras', function($scope,$http)
{
  ...
  $scope.selectProdBus = function(index,imgsrc)
  {
    ...
    $scope.unidad = $scope.lstaprdctbusq[index].UNIDAD_MEDIDAD.trim();
    $scope.precio = $scope.lstaprdctbusq[index].PRECIO_COMPRA != null ? $scope.lstaprdctbusq[index].PRECIO_COMPRA : '' ;
   ...
   $scope.idxRowProd = index;
    ...
    var data = {
      ...
      tipocambio:$scope.tipocambio!=''?$scope.tipocambio:null,
    	...
    	diascred:$scope.diascred != '' ? $scope.diascred : 0,
    	importe:$('#importe').val(),
    	iva:$scope.iva,
  ...
  $scope.regtracompraprod = function ()
  {
    ...
    $scope.listaCompras[$scope.listaCompras.length-1].ID_COMPRA = $scope.idcompra;
  	$scope.listaproductos = [];
    $scope.limpiarBox1();
  }
